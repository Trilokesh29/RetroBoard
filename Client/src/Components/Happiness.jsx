...
const Happiness = () => {
  ...
  const [name, setName] = useState(Config.getCurrentUserName());
  ...
  function removeHappiness(e) {
    if (window.confirm("Do you want to remove happiness ?")) {
      Config.getAxiosInstance()
        ...
        .then((result) => {
          if (-1 === result.data) {
            alert("[" + Config.getCurrentUserName() + "] does not have permission!");
          }
          updateAverage();
        })
        .then(() => {
            ...
            >
              <Row style={{ marginBottom: "30px" }}>
                ...
                <Col xs lg="2" style={{ paddingLeft: "5px", paddingRight: "0" }}>
                  <Button disabled={!shouldEnableSubmit()} variant="primary" type="submit" size="sm">
                    Submit Happiness
                  </Button>
                </Col>
              </Row>
